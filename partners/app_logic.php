...
if (isset($_POST['reset-password'])) {
  ...
  if (count($errors) == 0) {
    ...
    $sql = "UPDATE signup_partners_users set token='$token' where email='$email' ";
...
password, please go to the following link:  https://www.icargopacific.com/partners/newpass/ResetPassword.php?token=" . $token . "
        ...
        $token = mysqli_real_escape_string($db,$_POST['token']);
          ...
          else
          {
              ...
              if($new_password == $retype_pass)
              {
                  ...
                  {
                    //$new_pass = $new_pass);
                    $sql = "UPDATE signup_partners_users SET password='$new_password',token='' WHERE email='$email'";
                    $results = mysqli_query($db, $sql);
